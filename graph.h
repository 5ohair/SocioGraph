...
#ifndef _GRAPH_
#define _GRAPH_//prevents calling graph.h twice in main.cpp
...
public:
    ...
    int comments;
    int likes;
    vector<int> friends;//recs by interest and mutual friends
...
private:
    map<int, User> users;
    int count;//curr no of users
    int loggedIn;//curr user logged in
...
public:
    ...
    void mainPage(SocialNetwork& network);//login signup xit
